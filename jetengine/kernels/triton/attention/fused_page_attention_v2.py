...
# Migrated from layers.fused_page_attention_v2 (v2)
@triton.autotune(
    configs=[
...
def fused_kv_cache_attention_kernel(
    ...
    for start_m in range(0, block_len, BLOCK_M):
        ...
        q_ptrs = q_ptr + q_abs_offsets + head_idx * q_stride_head + tl.arange(0, BLOCK_DMODEL)[None, :] * q_stride_dim
        ...
        for logical_block_num in range(num_logical_blocks):
            ...
            num_tiles_in_block = (num_tokens_in_block + BLOCK_N - 1) // BLOCK_N
            if num_tiles_in_block > 0:
                block_start_n = 0
                ...
                for tile_idx in range(1, num_tiles_in_block):
                    block_start_n_next = tile_idx * BLOCK_N
                    actual_tile_size_next = min(num_tokens_in_block - block_start_n_next, BLOCK_N)
                    k_offsets_next = (block_start_n_next + tl.arange(0, BLOCK_N))[:, None] * k_cache_stride_pos
                    k_ptrs_next = k_cache_base + k_offsets_next + tl.arange(0, BLOCK_DMODEL)[None, :] * k_cache_stride_dim
                    k_tile_mask_next = (tl.arange(0, BLOCK_N)[:, None] < actual_tile_size_next) & dim_mask
                    k_next = tl.load(k_ptrs_next, mask=k_tile_mask_next, other=0.0).to(tl.float32)
                    v_ptrs_next = v_cache_base + k_offsets_next + tl.arange(0, BLOCK_DMODEL)[None, :] * v_cache_stride_dim
                    v_next = tl.load(v_ptrs_next, mask=k_tile_mask_next, other=0.0).to(tl.float32)
                    ...
                    alpha = tl.exp(m_i - m_ij_new)
                    p = tl.exp(qk - m_ij_new[:, None])
                    acc = acc * alpha[:, None] + tl.dot(p, v)
                    l_i = l_i * alpha + tl.sum(p, axis=1)
                    m_i = m_ij_new
                    k, v = k_next, v_next
                    actual_tile_size = actual_tile_size_next
                qk = tl.dot(q, tl.trans(k)) * sm_scale
                kv_attn_mask = tl.arange(0, BLOCK_N)[None, :] < actual_tile_size
                qk = tl.where(kv_attn_mask, qk, -float('inf'))
                m_ij_new = tl.maximum(m_i, tl.max(qk, axis=1))
        ...
        num_tiles_in_block = (block_len + BLOCK_N - 1) // BLOCK_N
        if num_tiles_in_block > 0:
            kv_block_idx = 0
            ...
            k_ptrs = k_ptr + k_abs_offsets + kv_head_idx * k_stride_head + tl.arange(0, BLOCK_DMODEL)[None, :] * k_stride_dim
            ...
            v_ptrs = v_ptr + v_abs_offsets + kv_head_idx * v_stride_head + tl.arange(0, BLOCK_DMODEL)[None, :] * v_stride_dim
            v = tl.load(v_ptrs, mask=k_mask, other=0.0).to(tl.float32)
            for tile_idx in range(1, num_tiles_in_block):
                kv_block_idx_next = tile_idx * BLOCK_N
                actual_block_size_next = min(kv_block_idx_next + BLOCK_N, block_len) - kv_block_idx_next
                k_abs_offsets_next = (seq_start_q + kv_block_idx_next + tl.arange(0, BLOCK_N)[:, None]) * k_stride_seq
                k_ptrs_next = k_ptr + k_abs_offsets_next + kv_head_idx * k_stride_head + tl.arange(0, BLOCK_DMODEL)[None, :] * k_stride_dim
                k_mask_next = (tl.arange(0, BLOCK_N)[:, None] < actual_block_size_next) & dim_mask
                k_next = tl.load(k_ptrs_next, mask=k_mask_next, other=0.0).to(tl.float32)
                v_abs_offsets_next = (seq_start_q + kv_block_idx_next + tl.arange(0, BLOCK_N)[:, None]) * v_stride_seq
                v_ptrs_next = v_ptr + v_abs_offsets_next + kv_head_idx * v_stride_head + tl.arange(0, BLOCK_DMODEL)[None, :] * v_stride_dim
                v_next = tl.load(v_ptrs_next, mask=k_mask_next, other=0.0).to(tl.float32)
                qk = tl.dot(q, tl.trans(k)) * sm_scale
                ...
                m_i = m_ij_new
                k, v = k_next, v_next
                actual_block_size = actual_block_size_next
            qk = tl.dot(q, tl.trans(k)) * sm_scale
            kv_attn_mask = tl.arange(0, BLOCK_N)[None, :] < actual_block_size
            qk = tl.where(kv_attn_mask, qk, -float('inf'))
            m_ij_new = tl.maximum(m_i, tl.max(qk, axis=1))
        ...
        o_ptrs = o_ptr + o_abs_offsets + head_idx * o_stride_head + tl.arange(0, BLOCK_DMODEL)[None, :] * o_stride_dim
